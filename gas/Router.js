...
var Router = (function () {
    ...
    try {
      switch (action) {
        ...
        case 'getInitialData': {
          ...
          var projects = isCreator
            ? ProjectModel.getAll()
            : ProjectService.getProjects(params.roomId);
          return ResponseUtil.success({
            projects: projects,
            defaultTasks: DefaultTaskModel.getAll(),
            canCreate: isCreator,
            isAdmin: isAdmin
          });
        }
      ...
      return ResponseUtil.error(err.message, err.code);
    }
  }
  ...
  function routeByData(ctx) {
    ...
    try {
      switch (action) {
        case 'getInitialData': {
          AuthService.verifyAccess(ctx.userId, ctx.roomId, null, ctx.displayName);
          var isCreator = UserModel.canCreate(ctx.userId);
          var isAdmin = MemberModel.isAdmin(ctx.userId);
          var projects = isCreator
            ? ProjectModel.getAll()
            : ProjectService.getProjects(ctx.roomId);
          return ResponseUtil.success({
            projects: projects,
            defaultTasks: DefaultTaskModel.getAll(),
            canCreate: isCreator,
