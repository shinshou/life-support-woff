...
function T_room_create_savesFields() {
  ...
  try {
    ...
    // RoomModel に deleteById がないため updateRow で対応
    try {
      var r = RoomModel.getById(roomId);
      if (r) SpreadsheetUtil.deleteRow(SpreadsheetUtil.getSheet('ルーム'), r._rowIndex);
    } catch (e) {}
  }
}
...
function T_auth_verifyAccess_autoRegistersNewRoom() {
  ...
  try {
    ...
    try {
      if (room) SpreadsheetUtil.deleteRow(SpreadsheetUtil.getSheet('ルーム'), room._rowIndex);
    } catch (e) {}
  }
}
...
function T_auth_verifyAccess_creatorBypassesProjectCheck() {
  ...
  RoomModel.create({ room_id: roomId, room_name: _TP + 'R' });
  var rRow = RoomModel.getById(roomId);

  try {
    // 作成者なので、紐づけなしでもエラーが出ないこと
    AuthService.verifyAccess(creatorId, roomId, projectId, '');
  } finally {
    try { if (rRow) SpreadsheetUtil.deleteRow(SpreadsheetUtil.getSheet('ルーム'), rRow._rowIndex); } catch (e) {}
  }
}
